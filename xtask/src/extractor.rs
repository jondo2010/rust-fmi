...
pub fn extract_model_data(dylib_path: &Path) -> Result<ModelData> {
    unsafe {
        ...
        let model_variables: schema::ModelVariables = fmi::schema::deserialize(variables_xml)
            .context("Failed to parse ModelVariables XML")?;

        let model_structure: schema::ModelStructure = fmi::schema::deserialize(structure_xml)
